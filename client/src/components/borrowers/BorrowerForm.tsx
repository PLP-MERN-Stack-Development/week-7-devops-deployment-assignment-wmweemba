...
import { Borrower } from '@/types'; // Ensure this is the updated type
...
export function BorrowerForm({ borrower, onClose }: BorrowerFormProps) {
    ...
    useEffect(() => {
        if (borrower) {
            setFormData({
                ...
                // Ensure joiningDate is correctly formatted for the date input
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (borrower) {
                // Pass borrower._id instead of borrower.id
                await updateBorrower(borrower._id, {
                    ...formData,
                    joiningDate: formData.joiningDate + 'T00:00:00.000Z' // Ensure backend receives ISO string
                });
            ...
            console.error('Form submission error:', error); // Log the full error for debugging
